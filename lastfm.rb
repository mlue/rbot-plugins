...
class LastFmPlugin < Plugin
  ...
  def search_venue_by(options)
    ...
    doc.root.elements.each("results/venuematches/venue") do |v|
      ...
      venue.long    = v.elements["location/geo:point/geo:long"].get_text.value.to_f
      venue.name    = v.elements["name"].get_text.value
      venue.city    = v.elements["location/city"].get_text.value
  ...
  def find_events(m, params)
    ...
    doc.root.elements.each("events/event"){ |e|
      ...
      country =  e.elements["venue"].elements["location"].elements["country"].get_text.value
      h[:location] = Underline + venue + Underline + " #{Bold + city + Bold}, #{country}"
      date = e.elements["startDate"].get_text.value.split
      h[:date] = Time.utc(date[3].to_i, date[2], date[1].to_i)
      h[:desc] = e.elements["description"].get_text.value
      h[:url] = e.elements["url"].get_text.value
      h[:attendance] = e.elements["attendance"].get_text.value.to_i
      artists = Array.new
  ...
  def find_artist(m, params)
    ...
    %w(playcount listeners).each do |e|
      t = first.elements["stats/#{e}"].get_text.value
      stats[e.to_sym] = t.gsub(/(\d)(?=(\d\d\d)+(?!\d))/, "\\1,")
    end
    summary = first.elements["bio"].elements["summary"].get_text.value
  ...
  def get_album(artist, album)
    ...
    # date = first.elements["releasedate"].get_text.value
    ...
    when :toptracks, :topalbums, :topartists, :weeklytrackchart, :weeklyalbumchart, :weeklyartistchart
      ...
      items = doc.root.get_elements("#{action}/#{type}").map do |item|
        ...
        when :weeklyartistchart, :topartists
          ...
          artist = item.elements["name"].get_text.value
